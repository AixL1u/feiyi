<template>
  <div>
    <div class="manage-header"><h2>商品分类管理</h2><el-button type="primary" @click="openDialog()">新增分类</el-button></div>
...
const loadData = async () => { const res = await adminApi.productCategoryList(); list.value = res.data }
const openDialog = (row) => { form.value = row ? { ...row } : { sortOrder: 0 }; dialogVisible.value = true }
const handleSave = async () => { await adminApi.saveProductCategory(form.value); ElMessage.success('保存成功'); dialogVisible.value = false; loadData() }
const handleDelete = async (id) => { await adminApi.deleteProductCategory(id); ElMessage.success('已删除'); loadData() }
onMounted(() => loadData())
</script>
